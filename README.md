...
Full reference documentation lives in **[docs/](docs/)** — start with the
[architecture overview](docs/architecture.md):

- [Architecture](docs/architecture.md) — the three pillars and end-to-end data flow.
- [Infrastructure (Terraform)](docs/infrastructure.md) — capacity, policies, RBAC, runner identity, variables/outputs.
- [Governance & Compliance](docs/governance-compliance.md) — the audit/tenant-settings/validate/deploy scripts and the CI/CD gate.
- [Management Lakehouse](docs/management-lakehouse.md) — the inventory & performance notebooks, Fabric API discoveries, and publishing.
- [Data Dictionary](docs/data-dictionary.md) — column-level schemas for the `lh_fabric_management` tables.
- [Operations](docs/operations.md) — auth, run order, scheduling, and private-output handling.
...
├── docs/                         # Full reference documentation (start at docs/README.md)
├── scripts/
...
│   ├── compare_tenant_settings.py # Baseline-vs-live tenant-settings diff
│   ├── list_workspaces.py        # Lists workspaces + their capacity
│   ├── push_notebook.sh          # Publishes a notebook to a Fabric workspace via az rest
│   └── create-bad-workspaces.py  # Demo script — generates 100 badly-named workspaces
├── notebooks/                    # Management-lakehouse pipelines (see docs/management-lakehouse.md)
├── audit-report.json             # Audit output (generated by audit_fabric.py; gitignored — real tenant data)
